...
class ProcessManager extends EventEmitter {
    ...
    start(task, isPriority = false) {
        ...
        const args = [];
        if (this.scraperScript) args.push(this.scraperScript);
        args.push(url);
        ...
        if (task.noSubdir) args.push('--no-subdir');
        ...
        child.stdout.on('data', (data) => {
            ...
            for (const raw of lines) {
                ...
                try {
                    ...
                    if (parsed.type === 'request_slot' && parsed.url) {
                        const fileKey = parsed.url;
                        this.slotManager.requestSlot(id, fileKey, () => {
                            if (entry.process && entry.process.stdin) {
                                entry.activeFiles.add(fileKey);
                                console.log(`[ProcessManager] Granting slot to ${id}: ${parsed.filename} (${fileKey})`);
                                entry.process.stdin.write(JSON.stringify({ action: "grant_slot", url: fileKey }) + "\n");
                            } else {
                                console.warn(`[ProcessManager] Cannot grant slot to ${id}: ${parsed.filename} - process/stdin missing`);
                            }
                        });
                    }

                    if (parsed.type === 'release_slot' && parsed.url) {
                        entry.activeFiles.delete(parsed.url);
                        this.slotManager.releaseSlot(id, parsed.url);
                    }
        ...
        child.on('close', (code) => {
            ...
            // Cleanup slots
            entry.activeFiles.forEach(fname => this.slotManager.releaseSlot(id, fname));
