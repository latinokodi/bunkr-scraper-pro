...
class BunkrScraperCore:
    ...
    def __init__(self, album_url, output_dir="downloads", progress_callback=None, max_workers=1, max_retries=5, links_only=False):
        ...
        self.cancel_flags      = {}
        self.slot_events       = {} # filename -> asyncio.Event
        self.loop              = None
        ...
        for line in sys.stdin:
            try:
                ...
                if data.get("action") == "skip" and "filename" in data:
                    ...
                    fname = data["filename"]
                    if self.loop and fname in self.slot_events:
                        self.loop.call_soon_threadsafe(self.slot_events[fname].set)
            except:
                pass
    ...
    async def _process_file(self, idx, file_info, total, album_dir, overall_start):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                self._emit("file_start", f"Starting: {filename}", filename=filename, fileurl=cdn_url)
                ...
                if filename in self.slot_events: del self.slot_events[filename]
